...
/*
 *
 * VCEmployeeSelectionFUN011.java
 *
...
/**
 * VCCash Drawer Selection Screen FUN011
...
public class VCEmployeeSelectionFUN011 extends AbstractViewController
{
	/**
	 * VCEmployeeSelectionFUN011 constructor comment.
	 */
	public VCEmployeeSelectionFUN011()
	{
		super();
	}
	/**
		...
		{ 
			...
			case (FundsConstant.GET_EMPLOYEE_DATA) :
				...
				try
				{
					getMediator().processData(
						getModuleName(),
						FundsConstant.GET_EMPLOYEE_DATA,
						aaData);
				}
				catch (RTSException leRTSEx)
				{
	 ...
	 * passes control.
	 * 
	 * @param java.util.Vector avPreviousControllers containing the 
	 *   names of the previous controllers 
	 * @param asTransCode String The TransCode
	 * @param aaData Objec The data object
	 */
	...
	{
		if (aaData != null
		 && ((FundsData) aaData).getEmployees() == null)
		{
			//Retrieve records
			this.setTransCode(asTransCode);
			processData(FundsConstant.GET_EMPLOYEE_DATA, aaData);
			return;
		}
		...
		if (getFrame() == null)
		{
			...
			{
				setFrame(new FrmEmployeeSelectionFUN011(
					laRTSDiagBox));
			}
			else
			{
