 ...
 * K Harrell	10/24/2005	Heading "Closeout end date/time" adjustment 
...
/**
 * Table model for screen FUN010.  Sets the column count and names
...
public class TMFUN010 extends javax.swing.table.AbstractTableModel
{
	...
	private final static String[] carrColumn_Name =
		{ "Cash Drawer", "Closeout end date/time" };
	// end defect 8379 	

	/**
	 * TMFUN010 constructor comment.
	 */
	public TMFUN010()
	{
		super();
	...
	public Object getValueAt(int aiRow, int aiCol)
	{
		...
		switch (aiCol)
		{
			case FundsConstant.FUN010_CASH_DRAWER :
			//if (aiCol == 0)			
				{
					return new Integer(
						((CloseOutHistoryData) cvVector.get(aiRow))
							.getCashWsId());

				}
			case FundsConstant.FUN010_CLOSEOUT_END_DATE_TIME :
				//else if (aiCol == 1)
				{
					return ((CloseOutHistoryData) cvVector.get(aiRow))
						.getCloseOutEndTstmp()
						+ CommonConstant.STR_SPACE_THREE
						+ (String) (((CloseOutHistoryData) cvVector
							.get(aiRow))
							.getCloseOutEndTstmp())
							.getClockTimeNoMs();
