package com.txdot.isd.rts.services.util.pdf417;
 ...
 * ------------	-----------	--------------------------------------------
 * K Harrell	03/21/2004	JavaDoc Cleanup. New Class.
 * 							Ver 5.2.0
 * B Brown		04/12/2011	Change resolution to a hard coded 37 as the
 * 							getToolkit().getScreenResolution() was 
 * 							causing an error when deployed to WAS.
 * 							defect 10670 Ver 6.7.1   						
 * ---------------------------------------------------------------------
 */
/**
 * PDF417 extends JPanel
 * 
 * @version	6.7.1		04/12/2011
 * @author	Michael Abernethy 
 * <p>Creation Date:	09/6/2002 8:54:23 
 */
public class PDF417 extends JPanel implements Serializable
{
	public String code;
	...
	protected String textCodes[][] = { { " ", "C26" }, {
			"!", "D10" }, {
			"\\", "D5" }, {
			"\r", "D11" }, {
			"\t", "D12" }, {
	...
	public PDF417()
	{
//		String lsOS = System.getProperty("os.name" );
...
//		{
//			resolution =
//				(int) ((double) getToolkit().getScreenResolution() / 2.54D);
//		}
//		System.out.println("lsOS = " + lsOS);		
//		System.out.println(
...
//				+ System.getProperty("java.awt.headless"));
		...
		narrowBarCM = 0.0D;
		widthBarCM = 0.0D;
		// changing resolution for WAS server side production of
		// receipt for WebAgent project
		resolution = 37;
// 		resolution was calcualted at 37 below		
// 		resolution =
//			(int) ((double) getToolkit().getScreenResolution() / 2.54D);
		barHeightPixels = 0;
		N = 2D;
	...
	protected void addBarWithHeight(
		Graphics g,
		int i,
		boolean flag,
		int j,
	...
	protected void paintCharWithHeight(
		Graphics g,
		String s,
		String s1,
		int i,
	...
	protected void paintPDF417(Graphics g)
	{
		...
		for (int j = 0; j < runtimeRows; j++)
		{
			...
			int l1 =
				(int) ((double) (PDFECLevel * 3)
					+ mod(runtimeRows - 1, 3D));
			paintCharWithHeight(
				g,
				"bwbwbwbw",
				toXSecuence(0x1fea8),
			...
			switch (i1)
			{
				case 0 : // '\0'
					paintCharWithHeight(
						g,
						"bwbwbwbw",
						toXSecuence(PDF417_BITS[i1][30 * j1 + k1]),
						currentY,
						barHeightPixels);
					break;
				case 1 : // '\001'
					paintCharWithHeight(
						g,
						"bwbwbwbw",
						toXSecuence(PDF417_BITS[i1][30 * j1 + l1]),
						currentY,
						barHeightPixels);
					break;
				case 2 : // '\002'
					paintCharWithHeight(
						g,
						"bwbwbwbw",
						toXSecuence(PDF417_BITS[i1][30 * j1 + l]),
						currentY,
						barHeightPixels);
					break;
			}
			for (int k = 0; k < PDFColumns; k++)
				paintCharWithHeight(
					g,
					"bwbwbwbw",
					toXSecuence(PDF417_BITS[i1][ai[i++]]),
					currentY,
					barHeightPixels);
			if (!truncated)
			{
	...
	protected int[] PDF417_compact_binary(byte abyte0[], int i)
	{
		...
		if (i < 1 || i > 30)
			i = 1;
		int k =
			(int) ((double) (2 + (abyte0.length / 6) * 5)
				+ mod(abyte0.length, 6D)
				+ (double) j);
		int l = k / i;
		if (mod(k, i) != 0.0D)
			l++;
		...
		for (j1 = 2; i1 + 5 < abyte0.length; j1 += 5)
		{
			...
			{
				ai[(j1 + 4) - l1] =
					biginteger.mod(new BigInteger("900")).intValue();
	...
	protected int[] PDF417_compact_numeric(byte abyte0[], int i)
	{
		...
		while (i1 < abyte0.length)
		{
			...
			int i2 = 0;
			for (;
				biginteger.compareTo(new BigInteger("0")) != 0;
				biginteger = biginteger.divide(new BigInteger("900")))
				ai1[i2++] =
					biginteger.mod(new BigInteger("900")).intValue();
	...
	protected int[] PDF417_compact_text(byte abyte0[], int i)
	{
		...
		while (k1 < abyte0.length)
		{
			...
			if (i1 >= 0)
			{
				...
				int j1 =
					(new Integer(s2.substring(1, s2.length())))
						.intValue();
				s2 = "" + s2.charAt(0);
				boolean flag3 = false;
				if (s2.compareTo(s) == 0)
